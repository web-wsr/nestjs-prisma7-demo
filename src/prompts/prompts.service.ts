...
export class PromptsService {
  ...
  async classifyText(text: string) {
    ...
    console.log(
      '🚀 ~ PromptsService ~ classifyText ~ examplePrompt:',
      examplePrompt,
    );
    ...
    console.log(
      '🚀 ~ PromptsService ~ classifyText ~ fewShotPrompt:',
      fewShotPrompt,
    );

    const chain = new StringOutputParser();
    const response = await fewShotPrompt
      .pipe(this.llm)
      .pipe(chain)
      .invoke({ input: text });

    // response 已经是 StringOutputParser 提取出的纯字符串
    return { text, sentiment: response.trim() };
  }
  // 代码审查
...
输出格式：总体评分（1-10分）+ 具体问题列表 + 改进代码片段`,
    ...
    const chain = prompt.pipe(this.llm).pipe(new StringOutputParser());
    console.log('🚀 ~ PromptsService ~ codeReview ~ chain:', chain);
    const result = await chain.invoke({ code, language });
    return { code, language, review: result };
  }
}
